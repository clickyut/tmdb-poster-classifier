...
class Products_images_model extends CI_Model {
    ...
    function set_primary_image($products_id, $image_id) {
        ...
        $image = $this->db->where('image_id', $image_id)->get('hhd_products_images')->row();
        if ($image) {
            $this->db->where('products_id', $products_id)
                     ->update('hhd_products', array(
                         'products_image' => $image->image_filename,
                         'folder_img' => dirname($image->image_path)
                     ));
        }
    }
        ...
        if ($image) {
            ...
            if ($image->is_primary == 1) {
                $next_image = $this->db->where('products_id', $image->products_id)
                                       ...
                                       ->row();
                if ($next_image) {
                    $this->set_primary_image($image->products_id, $next_image->image_id);
                } else {
                    // No more images, clear products table
                    $this->db->where('products_id', $image->products_id)
                             ->update('hhd_products', array(
                                 'products_image' => '',
                                 'folder_img' => ''
                             ));
    ...
    private function process_image_by_type($source_path, $extra) {
        ...
        if (in_array($extra, array(0, 1, 2))) {
            ...
            // Blu-ray: 1000x1149 (crop top/bottom)
            $target_width = 1000;
            $target_height = 1149;
            $need_crop = true;
        }
        ...
        if ($need_crop && abs($source_ratio - $target_ratio) > 0.1) {
            // Need to crop for Blu-ray
            // First resize to fit width
            $temp_height = $width / $target_ratio;
            
            if ($temp_height < $height) {
                // Crop height (top and bottom)
                $crop_top = ($height - $temp_height) * 0.2; // 20% from top to preserve title
                $crop_bottom = ($height - $temp_height) * 0.8; // 80% from bottom
                
                $config['x_axis'] = 0;
                $config['y_axis'] = $crop_top;
                $config['width'] = $width;
    ...
    function fix_missing_primary_images() {
        ...
        foreach ($products as $product) {
            ...
            if ($first_image) {
                $this->set_primary_image($product['products_id'], $first_image->image_id);
                $fixed++;
            }
        }
